...
from utils.coordUtils import coordConv, planeRot
...
def plotFan(sTime,rad,interval=60,fileType='fitex',param='velocity',filtered=False ,\
  ...
  **Example**:
  ...
  for i in range(len(myFiles)):
    ...
    lon, lat = coordConv(site.geolon,site.geolat,'geo',coords,dateTime=t) # Returns a pair of lists
  ...
  plotFactor = 1.05
  minx = x.min()*plotFactor     #since we don't want the map to cut off labels or
  miny = y.min()*plotFactor     #FOVs of the radars we should alter the extrema a bit.
  maxx = x.max()*plotFactor
  maxy = y.max()*plotFactor
  width = (maxx-minx)
  height = (maxy-miny)
  ...
  for i in range(len(files)):
    ...
    intensities, pcoll = overlayFan(scans,myMap,myFig,param,coords,gsct=gsct,site=sites[i],
        fov=fovs[i], fill=fill,velscl=velscl,dist=dist,cmap=cmap,norm=norm)
    for scan in scans:
